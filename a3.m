...
function ret = loss(model, data, wd_coefficient)
  ...
  wd_loss = sum(model_to_theta(model).^2)/2*wd_coefficient; % weight decay loss. very straightforward: E = 1/2 * wd_coeffecient * theta^6
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % This is the only function that youre expected to change. Right now, it just returns a lot of zeros, which is obviously not the correct output. Your job is to replace that by a correct computation.
  ...
  for j = 1:size(model.hid_to_class,2) % till number of hidden units
  	for i = 1:size(model.hid_to_class,1)  % till 10
  		ret.hid_to_class(i,j) = mean( hid_output(j,:).*  ( data.targets(i,:)- class_prob(i,:)));
    end
  end
  size(ret.hid_to_class)
  size(model.hid_to_class)
  ret.hid_to_class = ret.hid_to_class + wd_coefficient*model.hid_to_class; 
  inter = zeros(1,size(data.inputs,2));
  for i =1:size(model.input_to_hid ,1) % till hidden
  	for j = 1:size(model.input_to_hid,2) % till 256
  		for k=1:size(model.hid_to_class,1) % till 10 , can remove this loop easily
  			inter = inter + model.hid_to_class(k,i)*(data.targets(k,:)- class_prob(k,:));
  		end
  		ret.input_to_hid(i,j) = mean(hid_output(i,:) .* (ones(1,size(hid_output,2)) - hid_output(i,:)) .* inter .* data.inputs(j,:));
  	end
  end
